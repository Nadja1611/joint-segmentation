...
##################### accelerated segmentation algorithm bg constant############################
    def segmentation_step2denoisers_acc_bg_constant(self,f):
        ...
        self.x = proj_unitintervall(((x_old + self.tau*div(p1) - self.tau*(sm_diff1)**2 - self.tau*adjoint_der_Fid2_denoising(x_old, f, f2,self.r)))) # proximity operator of indicator function on [0,1]
        ######acceleration variables
        self.theta=1
        ###### 
        self.x_tilde = self.x + self.theta*(self.x-x_old)
        if self.verbose == True:
            fidelity = torch.mean((sm_diff1)**2*self.x) + torch.mean((sm_diff2)**2*(1-self.x))
            total = norm1(gradient(self.x))
            self.fid.append(fidelity.cpu())
            tv_p = norm1(gradient(self.x))
            self.tv.append(total.cpu())
...
##################### accelerated segmentation algorithm############################
    def segmentation_step2denoisers_acc(self,f):
        ...
        x_old = torch.clone(self.x)  
        # compute difference between noisy input and denoised image
        sm_diff1 = (f_orig-f1).float()#.unsqueeze(0)
        sm_diff2 = (f_orig-f2).float()#.unsqueeze(0)
        #filteing for smoother differences between denoised images and noisy input images
       # sm_diff1 = torch.nn.functional.conv2d(diff1, kernel,padding = 4).squeeze(0)
    ...
    def denoising_step_constant(self):
        ...
        for itr in range(self.denois_its):
           ...
           # loss = 0*torch.sum((output-y)**2*(1-mask)*loss_mask)/torch.sum((1-mask)*loss_mask)+torch.mean(((output-mu_r2)**2)*loss_mask)
    ...
    def denoising_step_r2(self):
        ...
        for itr in range(self.denois_its):
            ...
            loss = torch.sum((output-y)**2*(1-mask)*loss_mask)/torch.sum((1-mask)*loss_mask)+15*torch.mean(((output-mu_r2)**2)*loss_mask)
         #   loss = torch.mean(((output-mu_r2)**2)*loss_mask)
            if itr == 1000:
                print("loss_bg:"+str(loss))

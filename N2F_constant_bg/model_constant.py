...
class Discriminator(nn.Module):
    def __init__(self, nch_in, nch_ker=64, norm='bnorm'):
        ...
        self.dsc5 = CNR2d(8 * self.nch_ker, 1,                kernel_size=4, stride=1, padding=1, norm=[],  relu=[])
...
##################### accelerated segmentation algorithm bg constant############################
    def segmentation_step2denoisers_acc_bg_constant(self,f):
        ...
        #print("loss1:"+ str(torch.mean(diff1**2*self.x)))
        diff2 = (f_orig - self.mu_r2)
        #print("loss2:"+ str(torch.mean(diff2**2*(1-self.x))))
        ...
        self.x = proj_unitintervall(((x_old + self.tau*div(p1) - self.tau/2*(diff1)**2 + self.tau/2*diff2**2))) # proximity operator of indicator function on [0,1]
    ...
    def N2Fstep(self):
       ...
       # loss_mask = self.x
        ...
        while timesince <= 1000:
            ...
            loss1 = torch.sum((outputs-labello)**2*loss_mask)/torch.sum(loss_mask)
            loss = loss1
